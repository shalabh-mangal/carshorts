...
| 5 | **Skills** — each workflow codified/invokable | Consistency (playbook charter = skill #1) | **✅ SHIPPED** — `agents/skills.py` + `charters/skills/*.md` (`carshorts skills`): 5 codified workflows (source-footage, produce-video, ground-facts, publish-video, close-the-loop) with exact commands + gates; `run_agent` auto-routes a task to the matching skill and injects its steps, so the headless agent follows the canonical procedure instead of re-deriving it |
