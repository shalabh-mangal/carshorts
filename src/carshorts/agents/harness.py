...
`python -m carshorts.sourcing.crawl`) through the real pipeline and tallies:
  ...
  python -m carshorts.agents.harness                 # real Gemini, specs/ dir
  python -m carshorts.agents.harness --dir specs --throttle 30
...
from carshorts.adapters.llm import LLMClient, make_llm
from carshorts.core.models import FactCheckReport, SpecSheet, Verdict
...
def summarize(results: list[SheetResult]) -> str:
    ...
    if evaluated:
        ...
        lines.append("VERDICT: no sheets evaluated — crawl some specs first "
                     "(python -m carshorts.sourcing.crawl \"Car Name\").")
    lines.append("=" * 60)
    return "\n".join(lines)
...
def main() -> None:
    ...
    if not sheets:
        print(f"No spec sheets in {args.dir}/. Run: python -m carshorts.sourcing.crawl \"Tata Nexon\"")
        return
