"""The heartbeat — one command that runs the day.
  ...
  python -m carshorts.heartbeat              # run the day
  python -m carshorts.heartbeat --dry-run    # decide + report, change nothing
  python -m carshorts.heartbeat --status     # just the state of the channel
...
def run(dry_run: bool = False, no_agent: bool = False, max_pending: int = 2,
    ...
    if not status_only:
        try:
            from .retention_watch import run as watch
            watch(quiet=True)
    ...
    state = queue_state()
    try:
        from .calendar_plan import next_pending
        slot = next_pending()
    ...
    cmd = [sys.executable, "-m", "carshorts.pipeline", "--next"]
    if no_agent:
        cmd.append("--no-agent")
