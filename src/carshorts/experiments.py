"""Experiment ledger — turns hypotheses into knowledge, or refuses to.
  ...
  python -m carshorts.experiments                      # status board
  python -m carshorts.experiments --new "..." --variable length_s \
      --metric avg_view_pct --control 58 --treatment 35
  python -m carshorts.experiments --assign exp-001 --arm treatment --video VID
  python -m carshorts.experiments --evaluate exp-001
...
def board() -> str:
    ...
    if not data["experiments"]:
        return ("no experiments yet — create one:\n"
                '  python -m carshorts.experiments --new "shorter Shorts retain better" '
