"""Experiment scheduler — deliberate A/B across the content calendar.
  ...
  python -m carshorts.calendar_plan --build      # (re)generate the next 8 slots
  python -m carshorts.calendar_plan              # show the calendar
...
def show() -> None:
    if not CALENDAR.exists():
        print("no calendar — build one: python -m carshorts.calendar_plan --build")
        return
